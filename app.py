...
# Inicialización de estados base
...
# ... (MotorMatematico y funciones visuales se mantienen iguales) ...
...
def triple_bar(p1, px_val, p2, n1, nx, n2):
    st.markdown(f"""<div style="margin-top: 20px; margin-bottom: 25px; background: #161b22; padding: 15px; border-radius: 12px; border: 1px solid #30363d;"><p style='color:#00ffcc; font-size:0.9em; font-weight:bold; margin-bottom:10px;'>ANÁLISIS DE RESULTADO DIRECTO (1X2)</p><div style="display: flex; justify-content: space-between; font-size: 0.9em; margin-bottom: 10px; color: #eee;"><span>{n1}: <b>{p1:.1f}%</b></span><span>{nx}: <b>{px_val:.1f}%</b></span><span>{n2}: <b>{p2:.1f}%</b></span></div><div style="display: flex; height: 18px; border-radius: 9px; overflow: hidden; background: #333;"><div style="width: {p1}%; background: #00ffcc; box-shadow: 0 0 10px #00ffcc55;"></div><div style="width: {px_val}%; background: #444;"></div><div style="width: {p2}%; background: #3498db;"></div></div></div>""", unsafe_allow_html=True)

def dual_bar_explicit(label_over, prob_over, label_under, prob_under, color="#00ffcc"):
    st.markdown(f"""<div style="margin-bottom: 15px;"><div style="display: flex; justify-content: space-between; font-size: 0.85em; color: #eee; margin-bottom: 4px;"><span><b>{label_over}:</b> {prob_over:.1f}%</span><span><b>{label_under}:</b> {prob_under:.1f}%</span></div><div style="display: flex; background: #222; height: 10px; border-radius: 5px; overflow: hidden; border: 1px solid rgba(255,255,255,0.05);"><div style="width: {prob_over}%; background: {color};"></div><div style="width: {prob_under}%; background: rgba(255,255,255,0.05);"></div></div></div>""", unsafe_allow_html=True)
...
st.markdown("<style>.stApp { background-color: #0e1117; } .master-card { background: #161b22; padding: 25px; border-radius: 15px; border: 1px solid #30363d; margin-bottom: 20px; } .verdict-item { border-left: 4px solid #00ffcc; background: rgba(255,255,255,0.03); padding: 10px; margin-bottom: 8px; border-radius: 0 8px 8px 0; } .score-badge { background: #1c2128; padding: 8px; border-radius: 8px; border: 1px solid #30363d; margin-bottom: 5px; text-align: center; } .stButton>button { background: linear-gradient(90deg, #00ffcc 0%, #008577 100%); color: black !important; font-weight: bold; border: none; padding: 12px; border-radius: 10px; }</style>", unsafe_allow_html=True)

with st.sidebar:
    st.title("⚙️ PANEL CONTROL API")
    ligas_api = {"La Liga": 302, "Premier League": 152, "Serie A": 207, "Bundesliga": 175, "Ligue 1": 168, "UEFA Champions": 3, "Copa Libertadores": 13, "Brasileirão Serie A": 99, "Liga Mayor SLV": 601, "Copa Presidente SLV": 603, "FA Cup": 145}
    ...
    if eventos and isinstance(eventos, list) and "error" not in eventos:
        ...
        if st.button("⚡ SINCRONIZAR DATOS"):
            ...
            if standings and isinstance(standings, list):
                ...
                # TRUCO DE LIMPIEZA: Eliminamos las llaves para que Streamlit acepte el valor de la API como nuevo
                llaves_a_limpiar = ['p_liga_auto', 'lgf_auto', 'lgc_auto', 'vgf_auto', 'vgc_auto', 'nl_auto', 'nv_auto']
                for k in llaves_a_limpiar:
                    if k in st.session_state: del st.session_state[k]

                # Asignamos los valores frescos de la API
...
if st.button("🚀 PROCESAR ANÁLISIS ELITE", use_container_width=True):
    ...
    # Bloque de sugerencias y visualización...
    ...
    with tab_g:
        ...
        with gb: dual_bar_explicit("BTTS SÍ", res['BTTS'][0], "BTTS NO", res['BTTS'][1], color="#f1c40f")
    with tab_spec:
    ...
    with tab_m:
        ...
        fig = px.imshow(df_matriz, x=[str(i) for i in range(6)], y=[str(i) for i in range(6)], color_continuous_scale='Viridis', text_auto=".1f")
        st.plotly_chart(fig, use_container_width=True)

st.markdown("---")
st.markdown("<p style='text-align: center; color: #555; font-size: 0.8em;'>OR936 Elite v3.2 | Motor Dixon-Coles | Sincronización Forzada</p>", unsafe_allow_html=True)
