...
# =================================================================
# CONFIGURACIÓN DE API (Clave extraída de tu imagen)
# =================================================================
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            stats = {
                ...
                "tj": (data.get('cards', {}).get('yellow', {}).get('total', 0) or 0) / (played if played > 0 else 1),
            }
            return stats
    except:
        return None
...
with st.sidebar:
    ...
    liga_id = st.number_input("ID Liga (LaLiga=140, PL=39, CL=2)", 1, 1000, 140)
...
st.markdown("<h1 style='text-align: center; color: #00ffcc;'>OR936 ELITE ANALYSIS V3.0</h1>", unsafe_allow_html=True)
...
with col_l:
    ...
    ltj = c1.number_input("Tarjetas L", 0.0, 15.0, st.session_state.get('ltj', 2.3))
...
with col_v:
    ...
    vtj = c3.number_input("Tarjetas V", 0.0, 15.0, st.session_state.get('vtj', 2.2))
...
if st.button("🚀 PROCESAR ANÁLISIS COMPLETO", use_container_width=True):
    ...
    sugerencias = sorted([s for s in pool if 65 < s['p'] < 94], key=lambda x: x['p'], reverse=True)[:4]
...
st.markdown("<p style='text-align: center; color: #555; font-size: 0.8em;'>ProStats Engine OR936 v3.0 API-CONNECTED</p>", unsafe_allow_html=True)
