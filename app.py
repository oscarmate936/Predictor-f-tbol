...
st.markdown("""
    ...
    .top-result-card { background: linear-gradient(135deg, #1e2a3a 0%, #111827 100%); border: 1px solid #00ffcc; padding: 15px; border-radius: 12px; text-align: center; }
    ...
    .share-btn { width: 100%; background-color: #25D366; color: white !important; border: none; padding: 12px; border-radius: 10px; font-weight: bold; text-align: center; display: block; text-decoration: none; margin-top: 10px; transition: 0.3s; }
    .share-btn:hover { background-color: #128C7E; transform: scale(1.01); }
    </style>
...
if st.button("🚀 EJECUTAR ANÁLISIS PROFESIONAL", use_container_width=True):
    ...
    # -------------------------------------------------------------
    # NUEVO APARTADO: TOP 3 RESULTADOS Y COMPARTIR
    # -------------------------------------------------------------
    st.markdown("### 🏆 Veredicto: Marcadores más Probables")
    col_t1, col_t2, col_t3 = st.columns(3)
    
    top_scores_text = []
    for i, (score, prob) in enumerate(res['TOP']):
        top_scores_text.append(f"{score} ({prob:.1f}%)")
        with [col_t1, col_t2, col_t3][i]:
            st.markdown(f"""
                <div class="top-result-card">
                    <p style="color:#00ffcc; margin:0; font-size:0.9em;">PUESTO {i+1}</p>
                    <h2 style="margin:5px 0; color:white;">{score}</h2>
                    <p style="color:#aaa; margin:0;">{prob:.1f}% Prob.</p>
                </div>
            """, unsafe_allow_html=True)
    
    # Botón de Compartir en WhatsApp
    resumen_share = (
        f"📊 *Análisis ProStats OR936*\n"
        f"⚽ {nl} vs {nv}\n\n"
        f"🏆 *Top 3 Marcadores:*\n"
        f"1️⃣ {top_scores_text[0]}\n"
        f"2️⃣ {top_scores_text[1]}\n"
        f"3️⃣ {top_scores_text[2]}\n\n"
        f"📈 *Probabilidades 1X2:*\n"
        f"L: {res['1X2'][0]:.1f}% | E: {res['1X2'][1]:.1f}% | V: {res['1X2'][2]:.1f}%\n\n"
        f"👉 Analizado con ProStats Engine."
    )
    url_share = f"https://wa.me/?text={urllib.parse.quote(resumen_share)}"
    st.markdown(f'<a href="{url_share}" target="_blank" class="share-btn">📲 COMPARTIR ANÁLISIS EN WHATSAPP</a>', unsafe_allow_html=True)
    
    st.divider()

    # Pestañas de Mercados (Se mantienen intactas)
    ...
    with tab4:
        ...
        fig = px.imshow(df_matriz, labels=dict(x="Goles Visitante", y="Goles Local", color="% Prob"),
                        color_continuous_scale='Viridis', text_auto=".1f", aspect="auto")
        st.plotly_chart(fig, use_container_width=True)

st.markdown("<p style='text-align: center; color: gray; font-size: 0.8em;'>ProStats OR936 v2.3 - Engine Elite</p>", unsafe_allow_html=True)
